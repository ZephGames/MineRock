...
local function getCoinMultiplier(player)
	local mult = player:FindFirstChild("CoinMultiplier")
	if mult and typeof(mult.Value) == "number" then
		return mult.Value
	end
	return 1
end
...
local function sellAllOres(player)
    ...
    for oreName, config in pairs(OreConfig) do
        if type(config) == "table" and oreName ~= "SpawnWeights" and oreName ~= "RarityColors" then
            ...
            if item and item.Value > 0 then
                ...
                totalCoins = totalCoins + (amount * valuePer)
    ...
    if totalCoins > 0 then
        ...
        local finalCoins = math.floor(totalCoins * multiplier)
        coins.Value = coins.Value + finalCoins
        UpdateQuestSafe(player, "EarnCoins", finalCoins)
    end
end
...
local function awardTreasure(player, oreType, config, streakCount)
	...
	treasureChance = treasureChance + math.min(0.08, math.max(0, streakCount - 1) * 0.005)

    if math.random() < treasureChance then
		local coins = getOrCreateCoins(player)
		local coinReward = math.floor(((config and config.Value) or 1) * math.random(2, 5))
		local finalCoins = math.floor(coinReward * getCoinMultiplier(player))
		coins.Value = coins.Value + finalCoins
		UpdateQuestSafe(player, "EarnCoins", finalCoins)
