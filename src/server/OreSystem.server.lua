...
local function updateRockScale(rock)
	...
	local minVisualFrac = 0.1
	local healthTarget = initialScale * math.max(frac, minVisualFrac)

	-- Ensure the rock always visually shrinks on each scale update to show progress
	local minStep = initialScale * 0.015
	local targetScale = math.min(currentScale - minStep, healthTarget)
	targetScale = math.max(targetScale, initialScale * minVisualFrac)
