...
function PickaxeClient.init(tool)
	...
	local function getCurrentCooldown()
		...
		return cfg.Cooldown or 0.5
	end

	local function isRockInstance(inst)
		if not inst or not rockFolder then return false end
	...
	local function getRockPartUnderCursor()
		...
		if not mouseObj then return nil end
	...
	local function trySwing()
		...
		-- Always allow swing animation
		playSwingAnim()

		-- Only â€œhitâ€ if aiming at rock AND within range
		local hitPart = getRockPartUnderCursor()
		if not hitPart then
			return
		end

		local rockModel = getRockModelFromHit(hitPart)
		if not isWithinRange(rockModel, hitPart) then
			return
		end
